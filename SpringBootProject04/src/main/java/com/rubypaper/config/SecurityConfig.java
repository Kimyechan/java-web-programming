...
//    private DataSource dataSource;
    ...
    protected void configure(HttpSecurity http) throws Exception {
        ...
        // SecurityController 에 등록된 요청에 대해서 인증과 권한을 체크한다.
        ...
        // manager 경로에 있는 파일에 대한 요청은 인증애 성공했고 권한이 MANAGER 사용자만 접근하도록...
        http.authorizeRequests().antMatchers("/manager/**").hasRole("MANAGER");

        // admin 경로에 있는 파일에 대한 요청은 인증애 성공했고 권한이 ADMIN 사용자만 접근하도록...
...
//                .roles("ADMIN");*/
//
//        /*// 특정 아이디에 해당한느 회원 정보가 있는지 검색한다. ({noop} 접두사를 문자열 앞에 붙이면 암호하 작업을 하지 않는다.)
